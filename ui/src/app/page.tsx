...
function HomePageInner({
  ...
  const applyUserId = useCallback(
    async (assistantData: Assistant): Promise<Assistant> => {
      const userId = config.userId?.trim();
      if (!userId) return assistantData;
      const currentUserId =
        assistantData.config?.configurable?.["user_id"] ?? "";
      if (currentUserId === userId) return assistantData;
      try {
        const updated = await client.assistants.update(
          assistantData.assistant_id,
          {
            config: {
              ...
              configurable: {
                ...(assistantData.config?.configurable ?? {}),
                user_id: userId,
              },
            },
          }
        ...
        console.error("Failed to patch assistant user_id:", err);
  ...
  const fetchAssistant = useCallback(async () => {
    ...
    if (isUUID) {
      ...
      try {
        ...
        setAssistant(await applyUserId(data));
      } catch (error) {
      ...
      try {
        ...
        setAssistant(await applyUserId(defaultAssistant));
      } catch (error) {
        console.error(
  ...
  }, [client, config.assistantId, applyUserId]);
